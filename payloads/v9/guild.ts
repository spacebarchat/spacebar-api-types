...
import type { PublicUser } from '../../spacebar';
import type { APIGuildChannelResolvable } from '../../v9';
...
export interface APIGuild extends APIPartialGuild {
	...
	channels: APIGuildChannelResolvable[];
}
...
export interface APIMuteConfig {
	end_time: number;
...
export interface APIChannelOverride {
	...
	mute_config: APIMuteConfig;
	muted: boolean;
	channel_id: string | null;
}

export interface APIUserGuildSettings {
	channel_overrides: {
		[channel_id: string]: APIChannelOverride;
	} | null;
	message_notifications: number;
	mobile_push: boolean;
	mute_config: APIMuteConfig | null;
	muted: boolean;
